...
Každá položka v `config/course.yml` může pomocí `placement` určit umístění karty. Výchozí hodnota `materials` patří do hlavního přehledu materiálů; hodnota `schedule` vloží kartu na označené místo v rozvrhu. Volba `repository_link: false` skryje odkaz na soukromý zdrojový repozitář. Nastavení `card_resources: core` omezí odkazy na kartě na HTML a PDF skript, HTML a PDF prezentace a veřejná cvičení; výchozí `card_resources: all` navíc zobrazuje QMD zdroje, repozitář, data a přílohy. Skryté položky se pouze neukazují na kartě a zůstávají dostupné ve schváleném release balíčku. L00 používá `placement: schedule`, `repository_link: false` a `card_resources: core`, aby dobrovolná orientace měla jednoduchý studentský vstup bez odkazu na soukromý repozitář.
